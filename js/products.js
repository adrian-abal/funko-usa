...
var productsContainer = document.querySelector('.products__ul');
fetch('components/products.html').then(function (response) {
  ...
  var template = templateContainer.querySelector('#products__template');
  products.forEach(function (item) {
    var li = document.createElement('li');
    li.className = 'products__li';
    var product = template.content.cloneNode(true);
    var overviewLink = product.querySelector('.products__overview');
    var titleLink = product.querySelector('.products__a');
    var price = product.querySelector('.products__price');
    var category = product.querySelector('.products__category');
    var img = product.querySelector('.products__img');
    var tag = product.querySelector('.products__tag');
    ...
    img.src = "assets/img/products/".concat(item.media.front.src);
...
/* Products slider */
var productSliderMove = function productSliderMove() {
  var productArrows = document.querySelector('.products__arrows');
  var productArrowControl = productArrows.querySelectorAll('.arrows__control');
  var productsUl = document.querySelector('.products__ul');
  var productsLi = document.querySelectorAll('.products__li');
  var getProductsPerView = function getProductsPerView() {
  ...
  var productStep = 0;
  handleControls();
  var handleProducts = function handleProducts(direction) {
    ...
    var move = 100 / productsLi.length * productStep * -1;
    productsUl.style.transform = "translateX(".concat(move, "%)");
    handleControls();
  };
...
var overviewOnHover = function overviewOnHover() {
  var productsOverviews = document.querySelectorAll('.products__img');
  productsOverviews.forEach(function (element, i) {
    element.addEventListener('mouseover', function () {
      element.src = "assets/img/products/".concat(products[i].media.onHover.src);
    });
    element.addEventListener('mouseleave', function () {
      element.src = "assets/img/products/".concat(products[i].media.front.src);
    });
  });
};